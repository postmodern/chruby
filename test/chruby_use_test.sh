...
test_chruby_use()
{
	assertEquals "invalid RUBY_DIR" "$TEST_RUBY_DIR" "$RUBY_DIR"
	...
	assertEquals "invalid GEM_ROOT" "$TEST_RUBY_DIR/lib/ruby/gems/$TEST_RUBY_API" "$GEM_ROOT"
	...
	assertEquals "could not find ruby in $PATH" "$TEST_RUBY_DIR/bin/ruby" `which ruby`
}
