...
function chruby_reset()
{
	...
	PATH="${PATH#:}"; PATH="${PATH%:}"
	unset RUBY_ROOT RUBY_ENGINE RUBY_ENGINE_VERSION RUBY_VERSION RUBYOPT GEM_ROOT
	hash -r
}
...
puts "export RUBY_ENGINE_VERSION=#{Object.const_defined?(:RUBY_ENGINE_VERSION) ? RUBY_ENGINE_VERSION : ''};"
...
EOF
	...
	if (( UID != 0 )); then
		export GEM_HOME="$HOME/.gem/$RUBY_ENGINE/${RUBY_ENGINE_VERSION:-$RUBY_VERSION}"
